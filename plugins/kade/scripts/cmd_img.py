...
def _caller_base_dir() -> str:
    """
    Resolve base directory for relative user paths.
    Prefer caller cwd from request paths/system; fallback to process cwd.
    """
    try:
        p = paths()
        if isinstance(p, dict):
            for k in ("cwd", "pwd", "workdir"):
                v = p.get(k)
                if v:
                    return abspath_expand(v)
    except Exception:
        pass
    ...
    return abspath_expand(os.getcwd())


def resolve_user_path(user_path: str) -> str:
    """Resolve a user path relative to caller base dir."""
    user_path = str(user_path or "").strip()
    if not user_path:
        return ""
    if os.path.isabs(user_path):
        return abspath_expand(user_path)
    return abspath_expand(os.path.join(_caller_base_dir(), user_path))
...
def extract_ramdisk_lz4_cpio(
    ramdisk_file: str, out_dir: str, heartbeat_sec: int
) -> Tuple[str, str]:
    """
    ...
    run_cmd(
        [lz4_bin, "-d", ramdisk_file, cpio_path],
        cwd=None,
        stage="img:lz4_decompress",
...
def main():
    ...
    if not a:
        respond_error(
            "Usage: kade img <unpack|repack|extract-ramdisk|pack-ramdisk> ...", code=2
        )

    heartbeat = to_int(cfg("sync.heartbeat_sec", ""), 30) or 30

    base_dir = _caller_base_dir()
    log(f"[img] caller_base_dir={base_dir}")

    sub = a[0]
    rest = a[1:]

    # Default output base is workspace/img_out (but user --out is relative to caller_base_dir)
    ...
    if not workspace:
        ...
        home = (
            str(cfg("env.home", "") or "").strip()
            or os.environ.get("LYENV_HOME", "").strip()
        )
    ...
    out_base = (
        out_base if os.path.isabs(out_base) else os.path.join(workspace, out_base)
    )
    ensure_dir(out_base)

    unpack_py = _tool_path(
        str(cfg("img.tools.unpack_bootimg", "unpack_bootimg.py") or "unpack_bootimg.py")
    )
    repack_py = _tool_path(
        str(cfg("img.tools.repack_bootimg", "repack_bootimg.py") or "repack_bootimg.py")
    )
    mkbootimg_py = _tool_path(
        str(cfg("img.tools.mkbootimg", "mkbootimg.py") or "mkbootimg.py")
    )

    if sub == "unpack":
        if not rest:
            respond_error(
                "Usage: kade img unpack <boot.img|vendor_boot.img|system.img> [--out DIR]",
                code=2,
            )

        img = resolve_user_path(rest[0])
        ensure_file(img, "image")

        # Default output dir under workspace/img_out
        out_dir = os.path.join(out_base, os.path.basename(img) + ".unpacked")

        # parse --out (relative to caller base dir)
        if "--out" in rest:
            idx = rest.index("--out")
            if idx + 1 < len(rest):
                out_dir = resolve_user_path(rest[idx + 1])
        ensure_dir(out_dir)
        ...
        if os.path.basename(img).endswith("system.img"):
            ...
            respond_ok(
                "img unpack ok (system)",
                extra={
                    "outputs": [f"img={img}", f"out_dir={out_dir}", f"raw_img={raw}"]
                },
            )
            return
        ...
        run_cmd(
            ["python3", unpack_py, "--boot_img", img, "--out", out_dir],
            cwd=None,
            stage="img:unpack_bootimg",
            heartbeat_sec=heartbeat,
        )
    ...
    if sub == "extract-ramdisk":
        if not rest:
            respond_error(
                "Usage: kade img extract-ramdisk <ramdisk.lz4> [--out DIR]", code=2
            )

        ramdisk = resolve_user_path(rest[0])
    ...
    if sub == "pack-ramdisk":
        if not rest:
            respond_error(
                "Usage: kade img pack-ramdisk <ramdisk_dir> [--out FILE.lz4]", code=2
            )

        ramdisk_dir = resolve_user_path(rest[0])

        out_lz4 = os.path.join(out_base, "build.cpio.lz4")
        if "--out" in rest:
            idx = rest.index("--out")
            if idx + 1 < len(rest):
                out_lz4 = resolve_user_path(rest[idx + 1])

        out_file = pack_ramdisk_dir(ramdisk_dir, out_lz4, heartbeat)
        respond_ok(
            "pack-ramdisk ok",
            extra={"outputs": [f"ramdisk_dir={ramdisk_dir}", f"out_lz4={out_file}"]},
        )
        return
    ...
    if sub == "repack":
        ...
        if len(rest) < 2:
            respond_error(
                "Usage: kade img repack <orig_img> <work_dir> [--out new.img] [-- extra args...]",
                code=2,
            )

        orig_img = resolve_user_path(rest[0])
        work_dir = resolve_user_path(rest[1])
        ...
        if "--out" in rest:
            idx = rest.index("--out")
            if idx + 1 < len(rest):
                out_img = resolve_user_path(rest[idx + 1])
        ...
        respond_error(
            "repack_bootimg.py not found. mkbootimg.py requires device-specific args; please provide your own repack script.",
            code=2,
        )
